...
use crate::{
    ...
    backend_payment_integration_router::backend_payment_integration_router_with_postgres_pool,
...
pub fn build_payment_backend_router(host: Arc<PaymentServiceHost>) -> Router {
    let pool = host
        .database_pool()
        .as_postgres()
        .expect("payment backend-api requires an authoritative PostgreSQL pool")
        .clone();
    Router::new()
        ...
        .merge(backend_payment_integration_router_with_postgres_pool(pool))
}
